...
_discovery_setup() {
  ...
  V_RECONNECT_WINDOW_S="$(_min "$V_RECONNECT_WINDOW_S" "$DISCOVERY_V_RECONNECT_WINDOW_S")"
  printf 'short_windows=applied\nPOST_EXPIRY_WINDOW_S=%s\nRECOVER_WINDOW_S=%s\nW_POST_WINDOW_S=%s\nW_RECONNECT_WINDOW_S=%s\nV_RECONNECT_WINDOW_S=%s\n' \
    "$POST_EXPIRY_WINDOW_S" "$RECOVER_WINDOW_S" "$W_POST_WINDOW_S" "$W_RECONNECT_WINDOW_S" "$V_RECONNECT_WINDOW_S" > "$RUN_DIR/discovery-windows.txt"
