...
write_versions() { # SCENARIO CERT_SET: runs once, before baseline; dies naming any
  ...
  cfg="$(env | grep -E '^(LAB_|LINKERD_|GATEWAY_|ANCHOR_|ISSUER_|LEAF_|CONTROL_|REPLACEMENT_|PROBE_|OBSERVE_|POST_|RECOVER_|IMAGE_|PROFILE=|WEBHOOK_|EXTRA_|GATE_|OUTAGE_|FAULT_|S_HARD_|K_|POLICY_|SP_|NEW_|W_|N_|G_|DISCOVERY_)' | sort)" \
...
assert_no_keys() { # no private key in evidence, as PEM text or base64-encoded (key_scan,
  # the same scan lab/key-guard.sh runs before any run directory is committed)
  local hits
  hits="$(key_scan "$RUN_DIR")" \
    || die "private key material found in evidence, or the evidence could not be scanned: $hits"
}
