...
**Interfaces:**
- Consumes: runs `WI` and `WF` (Task 22), control `C` (Task 20), Task 18 helpers (`S=demos/cert-hygiene/scripts`).
- Produces: the evidence note, cited by Task 33.
...
- **Checks and recovery-branch artifacts:** `recover/branch.txt`, `recover/plain-facts.txt`, `recover/plain-render.txt` and `recover/plain-apply.txt` (both end `[exit 0]`; the validity rule `w-plain-render` requires it), `recover/plain-manifest.yaml`, `recover/plain-propagation.txt`, `controlplane/recover-plain.txt`, `admission/recovered/*-recover-plain.*`, and the `supplied` counterparts for branches ii and iii.

- [ ] **Step 3: Evidence per hypothesis**

Let the phases be: before T1; T1–T2 (proxy-injector expired); T2–T3 (proxy-injector and policy-validator expired); after T3 (all three). Map each `post-NNNN` directory (NNNN = seconds after T1, four digits by design) to a phase by comparing NNNN with the markers' times.

- **W1 (Ignore: each expired webhook's probe gets through):** in `WI`, for each phase from its webhook's expiry on:
  - (a) `inject-probe-*` created (`[exit 0]`), and its observed pod has no `linkerd-proxy`;
  - (b) `policy-invalid-*` accepted (`[exit 0]`) once the policy-validator has expired;
  - (c) `serviceprofile-invalid-*` accepted once the sp-validator has expired.

  Before each webhook's own expiry, its probe must still behave as at baseline; that is what isolates each webhook.
...
- **W4 (`linkerd check` goes fatal on each "… webhook has valid cert" row after that webhook's expiry):** `grep -h 'webhook has valid cert' $WI/checks/<tick named by each marker>-check.txt`, and the same rows one tick before each marker. The marker names a tick that started after the expiry, so its check ran after it.
- **W6 (observation: which recovery branch a plain upgrade produces for supplied credentials):** `cat $WI/recover/branch.txt $WF/recover/branch.txt`. Quote the facts lines, the propagation log's last line, the first line of `recover/plain-render.txt` (the command as run), and `grep -c redacted recover/plain-manifest.yaml` as proof the manifest was recorded. For ii and iii, quote `recover/supplied-facts.txt`. Give no verdict; write "Observed:" instead.
...
- **T1, T2, T3:** <UTC per run>; recovery began <UTC>, about 600 s after T3
...
In the status bullet, set the "Written up so far" sentence to: `Written up so far: the repeat of the issuer experiment and the webhook experiment.`
