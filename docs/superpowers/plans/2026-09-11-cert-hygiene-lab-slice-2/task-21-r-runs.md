...
- Produces: runs `RA` and `RB` (their stamps; named so they can't be confused with hypotheses R1–R4), each `evidence_valid=yes` at `H`. Task 27 reads both.

- [ ] **Step 1: First run**

Follow Task 20 Step 1 with `S=05-issuer-expiry`. It takes about an hour and a quarter.
