...
- Produces: runs `SS` and `SH`, each `evidence_valid=yes` at `H`. Task 31 reads both.

- [ ] **Step 1: S-staged**

Follow Task 19 Step 1 with `S=07-anchor-rotation-staged`. It takes under an hour: a 10-minute lead, then 11 steps with three gated restarts.
...
- `tail -n 1` of `steps/s03-upgrade.txt`, `steps/s07-upgrade.txt` and `steps/s10-upgrade.txt` each print `[exit 0]`. If one doesn't, the run is still committed, but record it: the guide's step failed in the lab, which Task 31 must report.
...
Follow Task 19 Step 1 with `S=08-anchor-rotation-hard`. It takes under an hour.

Sanity checks before committing:
- `head -n 1 runs/08-anchor-rotation-hard/<stamp>/s-hard/stage1-condition.txt` prints `result=met`. If it prints `result=timeout`, the run is invalid under a declared rule: follow Task 19 Step 1.6, second case.
