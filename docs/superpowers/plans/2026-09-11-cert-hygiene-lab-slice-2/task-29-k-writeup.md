...
**Interfaces:**
- Consumes: run `K` (Task 23), runs `RA` and `RB` (Task 20), `S=demos/cert-hygiene/scripts`.
- Produces: the evidence note, cited by Task 32 for the 60-day warning statements.
...
- The remaining validity for each command, computed from the calc files (show the subtraction: `issuer_not_after_epoch − check_started_epoch` for minus, `issuer_not_after_epoch − check_ended_epoch` for plus), in seconds and as days/hours/minutes.
