...
A correction is the highest-risk edit here, because it is made under the belief that the surrounding text was just checked — and a correction to a *source* document is riskier still, because work already running against it does not see the change. Every fix round got its own verification pass. Each of those passes found something — the whole-branch review is what caught the re-import — until the last one, which swept all seventeen article files for the class and came back empty. That is the first pass on this branch to find nothing, and it is the only evidence available that the class is actually closed rather than merely quiet.
