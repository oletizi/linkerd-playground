...
- Which of `linkerd check`'s rows *do* fail the command. Every transcript the threshold experiment recorded exited 0, but we drove none of that command's other checks to failure, so we can't say which ones exit non-zero. We know it happens — an expired trust anchor gave a non-zero exit, and `linkerd viz check` exits 1 on an expired tap certificate — but not where the line falls in general.
