...
Every expiry experiment below has been run, written up, and carried into [findings.md](findings.md). Its triage table no longer has an untested half. The two newest experiments — the negative controls in the last bullet — have been run and written up, but are not yet reflected in [findings.md](findings.md) or [sources.md](sources.md).
