...
Every experiment below has been run and written up. All but two have been carried into [findings.md](findings.md); the viz/tap write-up and the measured `linkerd check` boundary are new, and their triage rows have not been moved there yet.
...
- **The `linkerd check` 60-day warning:** run ten times — twice in the second round, then eight more as a bisect; nine of the ten are valid evidence, the tenth having been recorded against a dirty tree. Reproduced in both directions, and the boundary is now measured rather than bracketed by a day: it is not at 60 days. The check still printed its fatal row with 58 m 58 s *more* than 60 days of validity remaining, and only cleared to `√` at 1 h 4 m 3 s more — so the row is still fatal almost an hour's worth of validity past the 60-day mark, and therefore at exactly 60 days too; the warning starts roughly an hour before that mark is reached, not when it arrives. The pages should quote that 305-second bracket, never a single figure.
