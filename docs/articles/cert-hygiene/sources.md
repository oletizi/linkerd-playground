...
**Two of the entries below are negative controls, and they exist to stop the others being over-read.** In the unavailable-webhook runs and the refused-certificate run, *nothing expired* — the certificates involved were recorded as time-valid at every tick. They were run because every triage symptom this project had catalogued came from an expiry, so the triage table had never been tested at telling causes apart. Never cite either of them as evidence about expiry, and never merge their counts into the expiry experiments'.
...
- **Claims NOT supported — do not write these:**
  ...
  - Anything about Linkerd's default webhook credential model. These were lab-supplied static 24-hour certificates, as in the expiry runs.
...
- **2026-09-14, second entry:** Added the fourth round — the two experiments in which **nothing expired**. Until now every claim on these pages came from an expiry, and the triage table had never been tested at telling causes apart.
