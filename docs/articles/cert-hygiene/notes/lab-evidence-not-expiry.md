...
- **NI's observed fault span is 141 s** — first fault probe set `23:31:44Z` to last `23:34:05Z`. **NF's is 139 s** — `23:51:51Z` to `23:54:10Z`. Both come from the k3s journal entries quoted under N1, converted from the VM's local `-07:00` offset. That offset is checked against each run's own records, as the [reading guide](lab-evidence-reading-guide.md) requires: NI's first journal entry, `Sep 14 16:31:44`, against `admission/fault-0031/inject-probe-fault-plus10.observed.yaml`'s `creationTimestamp: "2026-09-14T23:31:44Z"`; NF's `Sep 14 16:51:51` against `admission/fault-0033/policy-valid-fault-plus10.observed.yaml`'s `creationTimestamp: "2026-09-14T23:51:51Z"`; G's `Sep 14 17:11:32` against `swap/settle-fault.txt`'s `2026-09-15T00:11:32Z state=refused`.
...
- The k3s journal agrees: six `Failed calling webhook, failing closed linkerd-proxy-injector.linkerd.io: … no endpoints available for service "linkerd-proxy-injector"` lines (`dispatcher.go:225]`), and `grep -c x509 logs/final/k3s-journal.txt` = **0** for the whole run.
...
The `seq=` counters run without a gap from first to last in all eight logs (NI 1–395 / 1–396 / 1–396 / 0–397; NF 1–394 / 1–394 / 1–394 / 0–396). The down windows used above are `23:31:14Z`–`23:34:16Z` (NI) and `23:51:18Z`–`23:54:19Z` (NF), each from the scale-down command to the `settled` marker. The four `probes/final/*-previous.log` files in each run each hold one line, `Error from server (BadRequest): previous terminated container "probe" … not found` — `kubectl` reporting that there is no previous container, which is consistent with `RESTARTS 0`, not a probe failure.
...
| Not Before / Not After | `Sep 15 00:01:03 2026 UTC` / `Sep 16 00:01:03 2026 UTC` | `Sep 15 00:11:25 2026 UTC` / `Sep 12 00:11:25 2036 UTC` |

`webhooks/<tick>.txt` corroborates the swap from a second direction and shows how narrow it was. `caBundle_sha256=36a7e8e5a549365965ae698ac146860af3a4ec65cf11322a5dc0847662457f90` on all three webhook configurations at `baseline`, `fault-plus10`, `post-4` **and** `verify` — the configured bundle was never touched. `secret_profileValidator_sha256` goes `0f432af5…` → `13cecb1d…` → back to `0f432af5…`, with the serials following (`D50E3F7E…` → `8EE7F22F…` → `D50E3F7E…`). `secret_proxyInjector_sha256=3d6a0798…` and `secret_policyValidator_sha256=9a07814a…` are unchanged at all four. `credential-plan.txt` reads `ok: 3 observed states walk the declared plan: A/I1/W1 A/I1/W2 A/I1/W1`, with the first and third webhook-state hashes identical (`f34599a9…`).
...
This is the same masking effect the lab already recorded for `linkerd viz check` in [`lab-evidence-tap-expiry.md`](lab-evidence-tap-expiry.md), where one expired tap certificate blanked the rest of the `linkerd-viz` section, and the same one recorded for `linkerd check` in [`lab-evidence-webhook-expiry.md`](lab-evidence-webhook-expiry.md), where the first expired webhook's fatal row hid the other two webhooks' rows for the rest of the run. Three different causes, three different sections, one behaviour. Any advice that tells a reader to "run `linkerd check` and look at the certificate rows" has to tell them that rows they cannot see have not passed.
...
- **Indistinguishable from the exit code alone: all three, under `Fail`.** Every rejected create is `[exit 1]` with `Error from server (InternalError): … failed calling webhook "<name>"`. The first two thirds of the sentence are identical across all three causes. What separates them is the clause after the final colon, and nothing else in the response.
