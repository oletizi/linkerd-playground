...
- **This moved the last untested triage row.** "`linkerd viz tap` and other viz features stop working / Viz tap API certificate expired" was in the *predicted, not tested* half of [findings.md](../findings.md)'s triage table, sourced to inference from Linkerd's code. This run tested it, and the row has since moved: that page now carries a single triage table in which every row has been reproduced, the predicted-not-tested table is gone, and this scenario's row lives in it as [Tap and viz](../findings.md#tap-and-other-viz-features-stopped-working). It moved with its qualifier rather than without it, which was the condition and still is: the row that replaced the inference names what `linkerd viz check` says, what the APIService says, and that tap kept streaming until the API server had to reconnect — not a flat "reproduced". Artifacts a reader can check: `apiservices/*.txt` and the three `*-conditions.json` snapshots, `tap/*.txt`, `viz-check/*.txt`, `certs/tap.txt`, `reconnect/`, and `logs/final/k3s-journal.txt`, all under `demos/cert-hygiene/runs/30-tap-expiry/20260914T091635Z` (read them with `tools/evidence.sh cat` or `fetch`).
