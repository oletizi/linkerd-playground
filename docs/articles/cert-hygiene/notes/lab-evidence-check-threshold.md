...
- The hypothesis's own conclusion — "the threshold lies between the two recorded remaining-validity values", i.e. between 5,183,374 s and 5,184,557 s in the run it was written for, a 1,183 s window — is **falsified**: the threshold is above 5,187,538 s, which is 2,981 s beyond the top of that window. (Both figures there are that run's *start* values, the 5,184,557 s included — the same number this note declines, above, to quote as a provable warn bound. That is deliberate and is a different use: the hypothesis named a window rather than a bound, and the window it names is at its widest when read from the start values, so falsifying it at its widest is the stronger claim. As a bound, that step still proves only its end value, 5,184,553 s.) A ±10-minute bracket around 1440h cannot contain this boundary, which is why the bisect had to vary the plus lifetime instead.
