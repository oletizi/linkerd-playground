...
The measured remaining-validity values land on the sides the condition requires: minus under 5,184,000 s, plus over it. The transcripts do not: all four print the identical fatal `‼ issuer cert is valid for at least 60 days` row, regardless of which side of the boundary the measured value fell on. Design § 13's K row requires **both** halves — the measured values and both command transcripts — to land on opposite sides of the boundary. Read strictly (the controller's ruling on this run), that is not the case here: **this run alone does not meet K's acceptance condition.** `k-remaining.txt`'s "ok" verdict does not settle this either way — it only checks the measured values against the threshold, and never checks which mark the transcript printed.
