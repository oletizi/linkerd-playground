...
The measured remaining-validity values land on the sides the condition requires: minus under 5,184,000 s, plus over it. The transcripts do not: all four of the run's transcripts — two per issuer, `linkerd check` and `linkerd check --proxy` for each — print the identical fatal `‼ issuer cert is valid for at least 60 days` row, regardless of which side of the boundary that issuer's measured value fell on. Design § 13's K row requires **both** halves — the measured values and both command transcripts — to land on opposite sides of the boundary. Read strictly (the controller's ruling on this run), that is not the case here: **this run alone does not meet K's acceptance condition.** `k-remaining.txt`'s "ok" verdict does not settle this either way — it only checks the measured values against the threshold, and never checks which mark the transcript printed.

Splitting the condition in two: the below-60-days half is met on its own — the transcript is fatal, as expected, whenever remaining validity is under 60 days. The above-60-days half — a transcript that actually clears to `√` — was not observed in this evidence run at all; the only place it was observed is the supplementary run below, which is not evidence. So K is **not fully reproduced** on the evidence this run provides: the warning side is reproduced, the pass side is not.
...
- The over-60-days half is **falsified**: at 5,184,557 s / 5,184,553 s remaining at the start of the command (plus), the check did not print `√`. Both of the plus step's transcripts — `linkerd check` and `linkerd check --proxy`, the only two this run records for that issuer — print the same fatal `‼` row as the minus step, quoting the issuer's own expiry (`k/plus-check.txt`, `k/plus-check-proxy.txt`). The run holds four transcripts in total, two per issuer; the other two belong to the minus (under-60-days) certificate.
...
**What the two runs together bound.** The evidence run alone only establishes that the threshold is *somewhere above* 60 days + 557 s. The supplementary run (below) adds an upper bound: with a 1464h30m issuer (61 days + 30 minutes), the plus checks passed. Together, the two runs bound the threshold to lie **above 60 days + 557 s and at or below 61 days + 30 minutes** — a window of (61 days + 30 min) − (60 days + 557 s) = 5,272,200 s − 5,184,557 s = 87,643 s, about 24 h 21 m, not the 1,183-second window K1 predicted. Only the evidence run's bound (above 60 days + 557 s) is evidence-backed; the upper bound depends on the supplementary run and is reported with that caveat.
...
Its plus rows, at roughly 5,272,169 s remaining at check end — about 24 h 29 m over the 60-day threshold (5,272,169 s − 5,184,000 s = 88,169 s), close to the configured margin of a 61-day-30-minute issuer over 60 days (24 h 30 m = 88,200 s); `k-remaining.txt`: `plus check had 5272169s left at its end (> 5184000s)` — **passed**:
```
√ issuer cert is within its validity period
√ issuer cert is valid for at least 60 days
...
- **K is not fully reproduced on this evidence.** Design § 13's K row needs both the measured remaining-validity values *and* both command transcripts to land on opposite sides of the boundary; in this run the transcripts don't flip (see "Acceptance condition" above). Only the below-60-days half — the warning firing — is reproduced. The reader-facing triage row may move to "reproduced" for that warning/below-60-days behaviour, but the check clearing to `√` above 60 days must **not** move to "reproduced": it was observed only in the non-evidence supplementary run.
- **The article must not claim the warning fires at exactly 60 days.** This lab's own evidence run shows a certificate with 557 seconds *more* than 60 days of remaining validity still getting the fatal `‼` row on both of its transcripts (`k/plus-check.txt` and `k/plus-check-proxy.txt`). Any statement of the boundary must say "somewhere above 60 days plus roughly 9 minutes, and observed to be at or below 61 days plus 30 minutes in this lab" — not "60 days," and not any other exact figure.
