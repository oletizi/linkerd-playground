...
No CSR is presented in the first 62 s after T_mark by any *existing* proxy — none of the six lab proxies' leaves was due for renewal until close to its own expiry at T+254s/T+257s (A2). The gap closes instead because two new pods are created right at T+60s (`applied probe-new`, `rolled restart-target`, `timeline.log:46-47`): `pods/post-1-probe-new-ddf9946ff-s4nn4.yaml:459` and `pods/post-1-restart-target-65847f75d-b5d5j.yaml:460` both record `startTime: "2026-09-12T13:13:22Z"` (T+60s). Each pod's `linkerd-proxy` init container requests its *initial* identity certificate on startup, and those two requests are exactly what produce the two refusals 2 s later: `logs/pre-recover/identity.txt:61-62`, both `time="2026-09-12T13:13:24Z"`, `CSR Identity : default.lab.serviceaccount.identity.linkerd.cluster.local`. The corresponding cluster events confirm the same pairing (`events/pre-recover.txt:235-236`: two `IssuerValidationFailed` events at `13:13:24Z`, immediately followed by further pairs at `13:13:34Z` and `13:13:44Z` as the two pods retry). From the first CSR after T_mark on, every one is refused: `grep -c 'could not process CSR' logs/pre-recover/identity.txt` returns **213**, and a scan of every `issued certificate` timestamp against the window `[T_mark, a-stage1-apply)` (`13:12:22Z`–`13:42:07Z`) finds **zero** issuances in that 1785 s span. The refusal reason names the anchor's own expiry each time: `Invalid After 2026-09-12 14:52:22 +0000 UTC` is the *issuer's* validity (still open), but the CA-chain check fails on the anchor, whose `notAfter` (`2026-09-12T13:12:22Z`) is quoted directly in the error (`... is after 2026-09-12T13:12:22Z`).
...
`probe-http` never fails at all, at any point in this run: `bash scripts/probe-lines.sh $A probe-http 2026-09-12T12:50:27Z | wc -l` returns **1557** deduplicated lines (`probes/pre-a-stage4/probe-http-586cfdddb4-rh878.log`, 1508 lines, plus `probes/final/probe-http-6bbf6fb7f-7hjsn.log`, 53 lines — the two non-empty per-pod files), and every one of them is `ok`; `grep -c ' fail '` over the same merged output returns 0. This is consistent with the reading guide's note that `probe-http` may reuse a connection rather than opening a new one per request.

`probe-tcp-stream`'s original connection (`conn=c645a483-1789217560`, opened at epoch `1789217560`, roughly 19.7 minutes before T_mark) is never observed to fail: its log, `probes/pre-a-stage4/probe-tcp-stream-5f4ccc76bc-hmn88.log`, runs `ok` every 2 s straight through the fault and stops at `seq=1514`, `2026-09-12T13:43:16Z ok`, `[exit 0]` — there is no `closed` line at all. That pod is restarted 2 s later as part of `a-stage4` (`timeline.log:113`, T+1856s), and the connection simply vanishes with it; this run has no evidence of what became of that specific TCP session, only that it was still `ok` immediately before the pod was killed. It is not evidence that anchor expiry leaves an open connection unaffected indefinitely — only that this one wasn't seen to fail before its pod was terminated.

Separately, the *new* `probe-tcp-stream` pod's *new* connection fails immediately: `probes/final/probe-tcp-stream-5b8bf84d44-gk4lx.log` shows `conn=7db84aeb-1789220602` opened at `2026-09-12T13:43:22Z` against `server.lab.svc.cluster.local:9000`, met with `Connection reset by peer`, and closed the same second (`closed socat_rc=1 fail-closed, not reconnecting`) — the probe does not retry. `gates/a-stage4.txt:21` (`check 2026-09-12T13:43:22Z server ... serving=no`) shows the new `server` pod itself was not yet serving at that same instant, which points to an ordinary restart-choreography race (client probing before the server side of the same rollout is ready) rather than anything TLS- or anchor-related. This single failed attempt is restart-adjacent, and this run does not fully explain it; it should not be read as evidence about anchor expiry.

## A3 — new connections fail; new pods never become Ready

**Verdict: confirmed.**

The pairs' post-expiry failures are established in A2 above (T+256s for both pairs, continuing — `bash scripts/probe-lines.sh $A probe-tcp-new 2026-09-12T12:50:27Z 2026-09-12T13:42:06Z` (bounded to before the `recover` marker, `timeline.log:106`) shows 759 `fail` lines against 713 `ok` lines for `probe-tcp-new`, and the identical 759/713 split for `probe-tcp-new-b`, i.e. every attempt after T+256s fails until recovery).

New pods created during the fault window never reach `Ready`:

- `probe-new` (applied at T+60s, `timeline.log:46`): its `Ready` condition is `status: "False"`, `reason: ContainersNotReady`, `message: 'containers with unready status: [linkerd-proxy idle]'` at `pods/post-1-probe-new-ddf9946ff-s4nn4.yaml` and unchanged through `pods/post-58-probe-new-ddf9946ff-s4nn4.yaml` (the last pre-recovery tick, `tick post-58` at `13:41:52Z`, `timeline.log:105` — T+1770s) — the same condition, word for word.
...
- **How failure spread:** existing connections were never observed to fail before anchor expiry made a new handshake necessary — `probe-http`'s (apparently reused) connection and `probe-tcp-stream`'s held-open connection stayed `ok` right through the fault (the latter's fate after its pod was later restarted for an unrelated reason, in recovery, is unobserved, not evidence either way). New connections into the endpoint with the earlier-expiring leaf (`server`, at T+254s) failed for both probe pairs at the same second (T+256s) — not staggered, because all six lab proxies' final leaves clustered within 3 s of each other in this run, contrary to A2's staggering prediction. New pods created during the fault window never became `Ready`, because identity refused their proxies' CSRs from T+62s on.
